    ...
    const detalle: ReportTabla[] = [
      ...
      },
      {
        ...
        columnas: [
          ...
          { clave: 'lote', titulo: t('crm.sales.origin_col'), solo: 'xlsx' }, { clave: 'oportunidad', titulo: t('crm.cart.opp'), solo: 'xlsx' },
          { clave: 'estado', titulo: t('crm.col.status'), solo: 'xlsx' },
        ],
        filas: ventas.map(v => ({ fecha: v.fecha, documento: v.documento, cliente: v.cliente, doc: v.cliente_documento, unidades: v.unidades, total: v.total, lote: v.lote ?? t('crm.sales.manual'), oportunidad: v.oportunidad,
